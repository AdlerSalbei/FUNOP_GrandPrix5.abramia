...
if !(missionNamespace getVariable ["grad_grandprix_gog_gameStarted",false]) exitWith {};
...
private _playerRank = _allPlayingPlayersScores find [player getVariable ["grad_grandprix_gog_currentScore",0],player];
    ...
    if (_i < _maxID) then {
        ...
        if (_i == 3 && _playerRank > 4) then {
            ...
            if (_i == 4 && _playerRank > 4) then {
                _score = player getVariable ["grad_grandprix_gog_currentScore",0];
                _unit = player;
